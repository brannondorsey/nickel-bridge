...
describe('handle (first-login username)', () => {
  ...
  it('patches preferences one key at a time, and refuses anything it does not recognise', async () => {
    ...
    // trick_clear_mode and trump_placement migrations in db.ts). foilTrumps
    // joins doubleTapBid in defaulting FALSE, for the opposite reason: there
    // was no prior behaviour for it to preserve.
    ...
    expect([
      ...
      me.user.foilTrumps,
    ]).toEqual([true, true, true, true, false, 'auto', 'suit', false]);
    ...
    expect([
      ...
      me.user.trumpPlacement,
      me.user.foilTrumps,
    ]).toEqual([false, false, true, true, false, 'auto', 'suit', false]);
    ...
    // trickClearMode and trumpPlacement are the string-valued preferences —
    // they patch and read back the same way, and a value outside their own
    // set is a 400 exactly like a non-boolean is for every switch above.
